...
Proof. 
  revert x y. simpl. induction x.
    ...
    - destruct y.
      -- reflexivity.
      -- simpl. intros. inversion H. destruct a, b.
          + simpl. rewrite IHx.
            ++ reflexivity.
            ++ exact H1.
          + simpl. rewrite IHx.
...
Proof.
  intros. destruct H. destruct H0. simpl. rewrite H. rewrite H0.
  ...
  assert(P_I_Rep: forall n, I :: repeat I n = repeat I (S n)).
  {
    intros. reflexivity.
  }
  ...
  assert(P_Rev_2: forall (b : B) (n : nat), rev (repeat b n) = repeat b n).
  {
    intros. induction n0.
      - reflexivity.
      - simpl. rewrite IHn0. rewrite repeat_cons. reflexivity.
  }
  
  assert(P_AddLr_Rep: forall n, AddLr (repeat I n) (repeat I n) I = repeat I n).
  {
    induction n0.
      - reflexivity.
      - simpl. rewrite IHn0. reflexivity.
  ...
  assert(P_Add_Rep: AddL (repeat I n) (repeat I n) = repeat I (pred n) ++ [O]).
  {
    induction n.
      - simpl. contradiction.
      - simpl. rewrite P_I_Rep. 
        unfold AddL. rewrite P_Rev_2. 
        simpl. rewrite P_AddLr_Rep. rewrite P_Rev_2. reflexivity.
  }
  
  assert(P_NotL_Add: forall n, NotL(repeat I n ++ [O]) = repeat O n ++ [I]).
  {
    induction n0.
      - reflexivity.
      - simpl. rewrite IHn0. reflexivity.
  ...
  induction n.
    - simpl. contradiction.
    - simpl. rewrite P_I_Rep. rewrite P_Add_Rep. simpl. rewrite P_NotL_Add.
    reflexivity.
Qed.
